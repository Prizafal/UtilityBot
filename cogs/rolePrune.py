...
class bulkRemove(commands.Cog):
    """
    /prunerole role_to_remove:<Role> keep_role:<Role> [dry_run]
    Removes `role_to_remove` from all members who DO NOT have `keep_role`.
    """
    ...
    @app_commands.command(
        name="prunerole",
        description="Remove a role from all users who don't have another specified role."
    )
    @app_commands.describe(
        role_to_remove="Role to remove from members",
        keep_role="Only members WITH this role keep the role; others lose it",
    ...
    async def prunerole(
        ...
        targets = [m for m in guild.members if (role_to_remove in m.roles and keep_role not in m.roles)]
...
async def setup(bot: commands.Bot):
    await bot.add_cog(bulkRemove(bot))
